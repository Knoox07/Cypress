...
it.only('Click link Unauthorized', () => {
  cy.viewport(1920, 1080)
  cy.visit('https://demoqa.com')
  cy.contains('Elements').click()
  cy.contains('Links').click()
  cy.get('[id="unauthorized"]').click()
  cy.get('[id="linkResponse"]').should('be.visible', 'Link has responded with staus 401 and status text Unauthorized')
  })
